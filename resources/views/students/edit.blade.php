...
@section('content')
    ...
    <form action="{{ route('students.update', $student) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat" required>{{ old('alamat', $student->alamat) }}</textarea>
            @error('alamat')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('no_hp') is-invalid @enderror" id="no_hp" name="no_hp" value="{{ old('no_hp', $student->no_hp) }}" required>
            @error('no_hp')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('hobi') is-invalid @enderror" id="hobi" name="hobi" value="{{ old('hobi', $student->hobi) }}" required>
            @error('hobi')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
